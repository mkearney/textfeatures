...
## create data frame with ID and title
nasa_title <- unique(data.frame(
  id = nasa$dataset$`_id`$`$oid`, 
  text = nasa$dataset$title, stringsAsFactors = FALSE))

## drop obs w/o IDs and duplicate texts (these could be rejoined later)
nasa_title <- subset(nasa_title, !is.na(id) & !duplicated(text))

## get text features
nasa_tf <- textfeatures(nasa_title, threads = 10)

## drop columns with zero variance
kp <- apply(nasa_tf[-1], 2, var)
nasa_tf <- nasa_tf[c("id", names(kp[kp > 0]))]
